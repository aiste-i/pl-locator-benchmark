...
import { appPaths, hrefSelector, pathRegex } from './helpers/app';
...
test.describe('Navigation and Filtering', () => {
  ...
  test('should navigate through main pages when logged out', async ({ page }) => {
    ...
    // Click Sign in
    await page.click(hrefSelector(appPaths.login()));
    await expect(page).toHaveURL(pathRegex(appPaths.login()));

    // Click Sign up
    await page.click(hrefSelector(appPaths.register()));
  ...
  test('should navigate through main pages when logged in', async ({ page }) => {
    ...
    await expect(page.locator(hrefSelector(appPaths.home())).first()).toBeVisible();
    await expect(page.locator(hrefSelector(appPaths.editor()))).toBeVisible();
    await expect(page.locator(hrefSelector(appPaths.settings()))).toBeVisible();
    await expect(page.locator(hrefSelector(appPaths.profile(user.username)))).toBeVisible();

    // Navigate to editor
    await page.click(hrefSelector(appPaths.editor()));
    await expect(page).toHaveURL(pathRegex(appPaths.editor()));

    // Navigate to settings
    await page.click(hrefSelector(appPaths.settings()));
    await expect(page).toHaveURL(pathRegex(appPaths.settings()));

    // Navigate to profile
    await page.click(hrefSelector(appPaths.profile(user.username)));
    await expect(page).toHaveURL(pathRegex(appPaths.profile(user.username)));
  });
  ...
  test('should paginate articles', async ({ page, request, browser }) => {
    ...
    await page.goto(appPaths.tag(uniqueTag));
  ...
  test('should show article count on profile tabs', async ({ page }) => {
    ...
    // Go to profile
    await page.goto(appPaths.profile(user.username));
  ...
  test('should handle empty states gracefully', async ({ page }) => {
    ...
    await page.goto(appPaths.profile(user.username), { waitUntil: 'load' });
