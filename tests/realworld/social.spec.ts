...
test.describe('Social Features', () => {
  ...
  test('should view own profile', async ({ page }) => {
    ...
    await expect(page.locator(hrefSelector(appPaths.settings())).filter({ hasText: 'Edit Profile Settings' })).toBeVisible();
  ...
  test('should view other user profile', async ({ page, request, browser }) => {
    ...
    await page.goto(appPaths.profile(targetUsername), { waitUntil: 'load' });
    await page.waitForSelector('h4', { timeout: 10000 });

    await expect(page.locator('h4')).toHaveText(targetUsername);
    await expect(page.locator('button:has-text("Follow")')).toBeVisible();
    await expect(
      page.locator(hrefSelector(appPaths.settings())).filter({ hasText: 'Edit Profile Settings' }),
    ).not.toBeVisible();
  ...
  test('should display favorited articles on profile', async ({ page }) => {
    ...
    await expect(page).toHaveURL(pathRegex(appPaths.profileFavorites(user.username)));
